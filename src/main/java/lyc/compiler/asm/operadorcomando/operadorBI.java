...
public class operadorBI implements IOperadorComando {
    ...
    public List<String> ejecutar(Tercetos terceto, HashMap<Integer, String> listaEtiquetas,  ArrayList<Tercetos> listaTercetos, Object trunc) {
        ...
        if (listaEtiquetas.get(destinoBI) == null) { // si no hay más instrucciones salta a la etiqueta final
            respuesta.add("\tJMP FINAL_LABEL" );
            respuesta.add("");
        } else {
            respuesta.add("\tJMP " + listaEtiquetas.get(destinoBI));
            respuesta.add("");
        }
