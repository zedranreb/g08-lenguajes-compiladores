...
public class operadorBE implements IOperadorComando {
    ...
    public List<String> ejecutar(Tercetos terceto, HashMap<Integer, String> listaEtiquetas, ArrayList<Tercetos> listaTercetos, Object trunc, ArrayList<Simbolo> tabla_simbolos) {
        ...
        if (listaEtiquetas.get(destinoBE) == null) { // si no hay más instrucciones salta a la etiqueta final
            respuesta.add("\tJE FINAL_LABEL" );
            respuesta.add("");
        } else {
            respuesta.add("\tJE " + listaEtiquetas.get(destinoBE));
            respuesta.add("");
        }
