...
public class operadorBLE implements IOperadorComando {
    ...
    public List<String> ejecutar(Tercetos terceto, HashMap<Integer, String> listaEtiquetas,  ArrayList<Tercetos> listaTercetos, Object trunc) {
        ...
        if (listaEtiquetas.get(destinoBLE) == null) { // si no hay más instrucciones salta a la etiqueta final
            respuesta.add("\tJBE FINAL_LABEL" );
            respuesta.add("");
        } else {
            respuesta.add("\tJBE " + listaEtiquetas.get(destinoBLE));
            respuesta.add("");
        }
